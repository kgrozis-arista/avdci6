#!/usr/bin/env python3
"""Interactive setup wizard for NetBox configuration.

This wizard configures the NetBox inventory management system:
- NetBox IP address (for device inventory and source of truth)
...
def configure_host_ip(
    *,
    ...
    if not prompt_yes_no(f"  Change {label}?"):
        print(f"  Keeping {label} as {current_ip}.\n")
        return
...
def main() -> int:
    """Run the NetBox setup wizard."""

    print("=" * 70)
    print("  avdci6 NetBox Setup Wizard")
    print("=" * 70)
    print()
    ...
    print("=" * 70)
    print("NetBox setup wizard complete.")
    print("=" * 70)
    print()
    print("Next steps:")
    print("  1. Review the updated inventory:")
    print("     cat avd_project/inventory/inventory.yml | grep -A 10 'NETBOX:'")
    print()
    print("  2. Test NetBox connectivity:")
    print("     ansible netbox -m ping -i avd_project/inventory/inventory.yml")
    print()
    print("  3. (Optional) Deploy device inventory to NetBox:")
    print("     make step6-netbox-onboard")
    print()
