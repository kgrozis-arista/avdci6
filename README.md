...
**Note:** Host configurations must be generated first using `make dev-host-build`. The playbook expects configurations to be located in `avd_project/AVD-information/intended/configs/`.
...
**Note:** Host configurations must be generated first using `make prod-host-build`. The playbook expects configurations to be located in `avd_project/AVD-information/intended/configs/`.
...
This orchestrates:
1. **Dev Reset Build** — Generate baseline reset configurations for dev fabric (spines, leafs) and hosts
2. **Dev Reset Deploy** — Deploy reset configs to dev CloudVision Portal
3. **Prod Reset Build** — Generate baseline reset configurations for prod fabric (spines, leafs) and hosts
4. **Prod Reset Deploy** — Deploy reset configs to prod CloudVision Portal
5. **Local Reset** — Restore local configuration files from backup
...
Restores `fabric_variables.yml` and `main.yml` from the `files/` backup directory.
...
3. **Deploy** (10 min timeout)
   - Refreshes repository state to latest commit (pulls build job output)
...
make step99-reset-prod       # Reset prod topology only
make reset-build             # Generate reset configs only
make reset-deploy            # Deploy reset configs to CloudVision only
make local-reset             # Restore local configuration files from backup
...
- Fixed CI/CD workflow deployment timing issue
  - Deploy jobs now refresh repository state before building (git fetch + git checkout)
  - Ensures deploy jobs receive configurations generated by build jobs
  - Applied to both dev-runner.yml and prod-runner.yml workflows
- Fixed host endpoint deployment path configuration
  - Updated host-deploy.yml output_dir to include `/intended` subdirectory
  - Aligns with actual configuration file location in AVD-information/intended/configs/
- Enhanced reset workflow integration
  - Added local-reset target to step99-reset orchestration
  - Reset now includes restoration of local configuration files from backup
- Documented documentation_dir configuration
  - Uncommented documentation_dir variable in FABRIC group_vars
  - Ensures eos_designs role outputs documentation to AVD-information/documentation/
- Updated README with CI/CD workflow improvements and reset workflow changes
